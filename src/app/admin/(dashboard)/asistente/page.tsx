    ...
    <div className="w-full max-w-full min-w-0 space-y-4 pb-12">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 px-4">
        <div className="flex items-center gap-3 sm:gap-4">
          <div className="w-10 h-10 sm:w-12 sm:h-12 bg-white shadow-xl shadow-gray-200/50 rounded-xl flex items-center justify-center border border-gray-100 relative shrink-0">
             <Bot size={20} className="text-black sm:w-[22px]" />
             <div className="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 sm:w-3 sm:h-3 bg-red-500 rounded-full border-2 border-white animate-pulse" />
          </div>
          <div>
            <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-black">Asistente AI</h1>
            <p className="text-[9px] sm:text-[11px] text-gray-500 font-bold uppercase tracking-widest opacity-70">Leads & Conversaciones</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 flex-wrap min-w-0">
          ...
          <div className="relative w-full sm:w-56 min-w-0 group">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 bg-white/60 backdrop-blur-md border border-white rounded-xl focus:outline-none focus:bg-white transition-all text-[12px] font-bold shadow-sm shadow-gray-100"
            />
          </div>
        </div>
      </header>
            ...
            >
              <div className="flex justify-between items-start mb-4 relative z-10">
                <div className="flex gap-3">
                   <div className="w-9 h-9 sm:w-10 sm:h-10 bg-gray-50 rounded-xl flex items-center justify-center border border-gray-100 group-hover:bg-black group-hover:text-white transition-all overflow-hidden shrink-0 shadow-sm">
                      {getSentimentIndicator(lead.sentiment)}
                   </div>
                   <div className="space-y-0.5 min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <h2 className="text-lg sm:text-xl font-bold text-black tracking-tight truncate">{lead.name}</h2>
                        {getPriorityBadge(lead.priority)}
                      </div>
                      <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5 text-[10px] sm:text-[11px] font-bold text-gray-400 tracking-tight">
                        <span className="flex items-center gap-1.5 truncate max-w-[150px] sm:max-w-none"><Mail size={10} className="text-blue-500" /> {lead.email}</span>
