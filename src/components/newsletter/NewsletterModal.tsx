...
/**
 * NewsletterModal — v3, Close button siempre visible
 * ─────────────────────────────────────────────────────────────────────────────
 * CORRECCIÓN MOBILE:
 * La X de cerrar estaba dentro del área overflow-y-auto y podía
 * desaparecer al hacer scroll. Solución: estructura flex-col en el card:
 *
 *   ┌─ Card (flex flex-col max-h-[92svh]) ─────────────┐
 *   │  Header flex-shrink-0 — drag handle + botón X     │ ← siempre visible
 *   │  Body   flex-1 overflow-y-auto — todo el contenido│ ← scrollable si necesario
 *   └───────────────────────────────────────────────────┘
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
export default function NewsletterModal() {
    ...
    // Auto-focus email
    useEffect(() => {
        const t = setTimeout(() => emailRef.current?.focus(), 340)
        ...
        >
            ...
            <motion.div
                ...
                className={[
                    ...
                    // ✅ flex-col para separar header fijo y body scrollable
                    'flex flex-col',
            ...
            >
                ...
                {/* ── HEADER — flex-shrink-0: siempre visible, nunca scrollea ── */}
                <div className="flex-shrink-0 px-5 pt-4 pb-2 sm:px-8 sm:pt-6 sm:pb-0">
                    {/* Drag handle — mobile only */}
                    <div
                        className="sm:hidden w-8 h-1 bg-gray-200 rounded-full mx-auto mb-3"
                        aria-hidden="true"
                    />
                    {/* Botón X — siempre visible en esta zona fija */}
                ...
                {/* ── BODY — scrollable si el contenido excede la altura ────── */}
                <div className="flex-1 overflow-y-auto px-5 pb-8 sm:px-8 sm:pb-8">
                    ...
                    <AnimatePresence mode="wait">
                        {status === 'success' ? (
                            ...
                            >
                                ...
                                <span>¡Gracias! Ya formas parte de nuestra comunidad. Volviendo…</span>
                            </motion.div>
                        ) : (
                            <motion.form
                                key="form"
